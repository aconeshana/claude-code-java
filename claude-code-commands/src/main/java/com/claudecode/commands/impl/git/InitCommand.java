...
/**
 * /init — Initialize CLAUDE.md project configuration.
 */
public class InitCommand implements Command {
    ...
    static final String CLAUDE_CODE_NEW_INIT_ENV = "CLAUDE_CODE_NEW_INIT";

/** Kept stable for compatibility with  — the external-user default. */
    static final String OLD_INIT_PROMPT = """
    ...
    /**
     * the
     * 8-phase interview workflow, active when {@link #CLAUDE_CODE_NEW_INIT_ENV}
     * is truthy.
     */
    static final String NEW_INIT_PROMPT = """
    ...
    public CommandMetadata metadata() {
        String description = isNewInitEnabled()
            ? "Initialize new CLAUDE.md file(s) and optional skills/hooks with codebase documentation"
            : "Initialize a new CLAUDE.md file with codebase documentation";
        return new CommandMetadata("init", description);
    }

    @Override
    public CommandResult execute(CommandContext context, String args) {

        String prompt = promptFor(isNewInitEnabled());
        HookDispatcher hooks = context.session().hookDispatcher();
        HookDispatcher.HookOutcome setup = hooks != null
            ? hooks.dispatchSetupWithOutcome("init") : HookDispatcher.HookOutcome.PROCEED;
        List<MessageContent> setupContext = setup.additionalContexts().stream()
        ...
        return CommandResult.forPrompt(PromptInvocation.builder(List.of(new TextBlock(prompt)))
            ...
            .userFacingName(name())
            .contentLength(0)
            .precedingUserMessages(setupContext)
            .build());
    }

    /** Prompt selection — package-private so tests can cover both gate states. */
    static String promptFor(boolean newInit) {
        return newInit ? NEW_INIT_PROMPT : OLD_INIT_PROMPT;
    }

    static boolean isNewInitEnabled() {
        return EnvUtils.isEnvTruthy(
            SubprocessEnvironment.get(CLAUDE_CODE_NEW_INIT_ENV));
    }
}
