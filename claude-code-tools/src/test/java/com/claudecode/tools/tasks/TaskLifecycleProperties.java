...
class TaskLifecycleProperties {
    ...
    void generatedIdAlwaysHasCorrectPrefix(@ForAll("taskTypes") TaskType type) {
        ...
        assertTrue(Strings.CS.startsWith(id, type.prefix()),
     ...
     * (In-memory store — the previous {@code new TaskStore} here silently
     * wrote task files into the developer's real ~/.claude/tasks on every run.)
     */
    @Property(tries = 20)
