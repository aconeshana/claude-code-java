...
/** Returns true only for a established allowlisted MCP search/read tool. */
    public static boolean isCollapsible(String qualifiedToolName) {
        String toolName = unqualifiedToolName(qualifiedToolName);
        if (toolName == null) return false;
        String normalized = normalize(toolName);
        return SEARCH_TOOLS.contains(normalized) || READ_TOOLS.contains(normalized);
    }

    static String normalize(String name) {
        return CAMEL_BOUNDARY.matcher(name).replaceAll("$1_$2")
            .replace('-', '_')
            .toLowerCase(Locale.ROOT);
    }

    private static String unqualifiedToolName(String qualifiedToolName) {
        if (qualifiedToolName == null || !Strings.CS.startsWith(qualifiedToolName, "mcp__")) return null;
        int separator = qualifiedToolName.indexOf("__", "mcp__".length());
        if (separator < 0 || separator + 2 >= qualifiedToolName.length()) return null;
        return qualifiedToolName.substring(separator + 2);
    }
}
