Review target: GitHub pull request `$ARGUMENTS`.

Gather this target's diff with (instead of any local `git diff`):
1. `gh pr view $ARGUMENTS --json title,body,author,baseRefName,headRefName,state,additions,deletions,changedFiles,labels` for context
2. `gh pr diff $ARGUMENTS` for the unified diff
