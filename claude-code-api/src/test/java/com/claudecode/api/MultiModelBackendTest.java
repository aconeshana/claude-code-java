...
class MultiModelBackendTest {
    ...
    @Test
    void openAiCompatClientCreateMessage() throws Exception {
        String jsonResponse = """
            {
                "id": "chatcmpl-123",
                "model": "gpt-4",
        ...
        try (MockWebServer server = new MockWebServer()) {
            ...
            server.start();
            var config = new ApiConfig.OpenAiConfig("sk-test", "gpt-4", server.url("/v1").toString());
    ...
    @Test
    void openAiCompatClientStreamReturnsEvents() throws Exception {
        String sseResponse = "data: {\"choices\":[{\"delta\":{\"content\":\"Hi\"}}]}\n\ndata: [DONE]\n\n";
        try (MockWebServer server = new MockWebServer()) {
            server.enqueue(new MockResponse.Builder()
                .code(200).addHeader("Content-Type", "text/event-stream").body(sseResponse).build());
            server.start();
            var config = new ApiConfig.OpenAiConfig("sk-test", "gpt-4", server.url("/v1").toString());
            var client = new OpenAiCompatClient(config, HttpClientFactory.shared());
            var request = CreateMessageRequest.builder().model("gpt-4").build();
            ...
            // The adapter emits a message envelope followed by the text block.
            List<StreamEvent> events = new ArrayList<>();
            stream.forEachRemaining(events::add);
            assertInstanceOf(StreamEvent.MessageStart.class, events.getFirst());
            assertTrue(events.stream().anyMatch(StreamEvent.ContentBlockStart.class::isInstance));
            assertTrue(events.stream().anyMatch(StreamEvent.ContentBlockStop.class::isInstance));
        }
    }
