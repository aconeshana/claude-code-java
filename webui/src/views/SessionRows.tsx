 ...
 *   title, trailing relative time that swaps to row actions on hover.
 ...
 * session rename/fork/archive menus, drag-and-drop reorder wiring, HoverCard
 * previews, schedule indicator, subagent/pending-interaction statuses, the
 * visually-hidden status labels. Close-headless is the one row action this
 * gateway has that upstream expresses as a menu item instead.
 */
import clsx from 'clsx'
import {
  IconFolderClose16, IconFolderOpen16, IconPlusOutline16,
  IconTriangleRightFill14, relativeTime, StateDot,
} from '@primitives'
import type { StateDotState } from '@primitives'
 ...
 * the row menu (rename/fork/archive), drag wiring, HoverCard, and the
 * schedule/subagent indicators. The trailing cell keeps upstream's hover
 * swap: relative time by default, row actions on hover. `onClose` is this
 * gateway's own action (closing a headless session) rendered with upstream's
 * bare 16px trailing-icon grammar.
 */
export function SessionNodeItem({ session, selected, now, onOpen, onClose, t }: {
  session: CatalogSession
  selected: boolean
  ...
  return (
    ...
    >
      ...
      <span className={css.title}>{displayTitle(session)}</span>
      ...
      <span className={css.rowActions}>
        {session.headless_open && !session.active && (
          <button
            ...
            aria-label={t('actions.close.aria', { name: displayTitle(session) })}
            onClick={(e) => {
              e.stopPropagation()
              onClose(session)
            }}
