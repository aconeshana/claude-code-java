...
| `TurnUsagePanel.tsx` | `ui-chat/src/client/chat/TurnUsagePanel.tsx` | Adapted: `usage` is the gateway's `TurnUsage` wire type (snake_case buckets, always-present) instead of upstream's attempt-lifecycle `TurnTokenUsage` (optional buckets); the routes row (provider/model attribution) and the reasoning-tokens row are cut — no data source exists for either. The TPS/TTFT dialog rows stay prop-driven and simply stay absent until a data source lands (upstream's facts-absent-rows-omitted semantics). `TurnTimePanel` in the same file keeps its optional `tokensPerSecond`/`ttftMs` props. |
