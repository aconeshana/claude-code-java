...
class CompactSafetyProperties {
    ...
    void compactBoundaryIsCorrectlyFormatted(
        ...
        assertEquals(compactType, marker.compactMetadata().trigger());
        assertEquals((long) tokenCount, marker.compactMetadata().preTokens());
    ...
    private Arbitrary<List<Message>> combineRounds(List<Arbitrary<List<Message>>> rounds) {
        ...
        Arbitrary<List<Message>> combined = rounds.getFirst();
