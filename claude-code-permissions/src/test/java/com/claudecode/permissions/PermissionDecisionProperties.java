...
class PermissionDecisionProperties {
    ...
    void sameRulesAndInputProduceSameDecision(
        ...
        PermissionDecision first = engine.evaluateDetailed(toolName, input, context).decision();
        PermissionDecision second = engine.evaluateDetailed(toolName, input, context).decision();
    ...
    void denyRulesTakePrecedenceOverAllowRules(
        ...
        PermissionDecision decision = engine.evaluateDetailed(toolName, input, context).decision();
    ...
    void addDirectoriesPreservesRulesAndMode(
        ...
        for (Path dir : original.additionalDirs().keySet()) {
            assertTrue(updated.additionalDirs().containsKey(dir),
    ...
    void mutationOperationsAreImmutable(
        ...
        List<Path> origDirs = List.copyOf(original.additionalDirs().keySet());

        // Perform mutations
        original.setMode(newMode);
        original.addRules(newRules);
        original.replaceRules(newRules);
        original.removeRules(_ -> true);
        ...
        assertEquals(origDirs, List.copyOf(original.additionalDirs().keySet()), "Original dirs must not change");
    }
    ...
    Arbitrary<ToolPermissionContext> permissionContexts() {
        return Combinators.combine(
            ...
            ToolPermissionContext.builder()
                ...
                .mode(mode)
                .rules(rules)
                .additionalDirs(dirMap(dirs))
                .build()
        );
    }

    /** Builds a dir→SESSION map for constructing a ToolPermissionContext in tests. */
    private static Map<Path, RuleSource> dirMap(List<Path> dirs) {
        Map<Path, RuleSource> map = new LinkedHashMap<>();
        for (Path d : dirs) {
            map.put(d, RuleSource.SESSION);
        }
        return map;
    }
